...
class CommentThreadScreen extends ConsumerStatefulWidget {
  ...
  const CommentThreadScreen({Key? key, required this.parentComment})
      : super(key: key);
...
class _CommentThreadScreenState extends ConsumerState<CommentThreadScreen> {
  ...
  void _handleSubmit() {
    ...
    if (widget.parentComment.parentCommentId == "0") {
      ...
      ref
          .read(commentsProvider(widget.parentComment.postId).notifier)
          .incrementReplyCount(widget.parentComment.id);
    } else {
      // It's a nested reply, so it lives in repliesProvider of its parent
      ref
          .read(repliesProvider(widget.parentComment.parentCommentId).notifier)
          .incrementReplyCount(widget.parentComment.id);
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.parentComment.parentCommentId == "0") {
      parentListState =
          ref.watch(commentsProvider(widget.parentComment.postId));
    } else {
      parentListState =
          ref.watch(repliesProvider(widget.parentComment.parentCommentId));
    }
                ...
                children: [
                  ...
                  Expanded(
                    child: AppTextField(
                      ...
                      isPassword: false,
                      hint: "Reply to ${widget.parentComment.author.name}...",
                    ),
                  ),
                  TextButton(
